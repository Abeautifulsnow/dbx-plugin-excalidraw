// Byte / base64 / dataURL helpers plus a SHA-256 implementation that works
// without WebCrypto (crypto.subtle requires a secure context, which cannot be
// assumed inside the DBX sandbox iframe).
...
export function parseDataURL(dataURL: string): { mimeType: string; bytes: Uint8Array } {
  ...
  return { mimeType, bytes: base64ToBytes(dataURL.slice(comma + 1)) };
}
